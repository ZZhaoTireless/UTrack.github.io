...
describe('Student Unit Tests', function() {
    ...
    var data_sample = new ActivityData('Sample_ACT', {energy:3,stress:4,happiness:5}, 59);

    it('Add A New Activity With Sample Data', function() {
        var ACT_MOD = new ActivityCollectionModel(),
            tst_listener = sinon.spy();

        ACT_MOD.addListener(tst_listener);
        ACT_MOD.addActivityDataPoint(data_sample);
        expect(tst_listener.called, 'Listener of AcivityModel should be called').to.be.true;
    });

    it('Add data test for ActivityCollectionModel', function() {
        var ACT_COL_MOD = new ActivityCollectionModel();

        var addFnSpy = sinon.spy(ACT_COL_MOD, 'addActivityDataPoint');

        ACT_COL_MOD.addActivityDataPoint(data_sample);
        expect(ACT_COL_MOD.getActivityDataPoints().indexOf(data_sample) === -1, 'ActivityCollectionModel should store data').to.be.false;
        expect(addFnSpy.called, 'ActivityCollectionModel adding should be called').to.be.ok;
    });

    it('Select graph with INVAILD name', function() {
        var NEW_GRPH = new GraphModel();
        var tst_listener = sinon.spy();

        NEW_GRPH.addListener(tst_listener);
        NEW_GRPH.selectGraph("wrong_graph");

        expect(tst_listener.called, 'Listener of GraphModel should not be called').to.be.false;
    });
